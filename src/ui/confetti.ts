...
const COLORS = ['#8B5CF6', '#2DD4BF', '#F59E0B', '#34D399', '#60A5FA', '#F472B6', '#E8EEF4'];
...
export function launchConfetti(durationMs = 4200): ConfettiHandle | null {
  ...
  if (reduce) {
    // Static sprinkle layer — celebration without motion sickness.
  ...
  // Big opening burst + side cannons + continuous rain
  spawn(70);
  spawn(40, true);
  ...
  raf = requestAnimationFrame(frame);
  // Auto-stop a bit after duration in case RAF chain ends early
